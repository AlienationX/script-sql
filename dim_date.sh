...
from   (
        select date as date_str,
               ...
               concat(substr(date,1,4),'-',substr(date,6,2)) as yearmonth_str,
               ...
               concat(date_sub(date,pmod(datediff(date,'1900-01-01'),7)),'~',date_add(date,6-pmod(datediff(date,'1900-01-01'),7))) as week_concat
